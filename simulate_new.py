...
def select_separator(T):
    ...
    for node in T.nodes():
        ...
        if max(component_sizes) <= 2 * len(T.nodes()) / 3:
            sep = node
            break
    if not sep:
...
def simulate_treewidth_algorithm(G, root):
    ...
    for cov in random.sample(list(G.nodes()), 100):
        ...
        for t in range(100):
            ...
            P = find_order(G, {u:0 for u in G.nodes()}, P_, root)
            selected_component, s, extra_it = select_component(G_it, P[::-1], CV, s)
            I = get_ideal(G_it,V,s)
            V = {v: not (v in selected_component and v in I) for v in G_it.nodes()}

            it += extra_it
            #print(size, P, extra_it)
...
def greedyAppReduceAll(G, W, V_, root, K, slim, wlim, plim, calcRobust, visitRobust):
    ...
    while len(ans) < K:
        ...
        S = {u:0 for u in G.nodes()}
        SS = {u:0 for u in G.nodes()}
        for v in G.nodes():
            if not V[v]:
                ss = get_size(G, V, v)
...
def simulate_robust_randtree(G, root, map_prev):
    iters = []
    #map_prev = {}
    N = len(G.nodes())
    node_it = 0
    ...
    for r in random.sample(list(G.nodes()), 100):
        ...
        for t in range(100):
            ...
            if nn in map_prev.keys():
                ...
                if R > N / 10:
                    P, sP = greedyAppReduceAll(G, W, V, root, 1, R / 3, 1e8, 0, True, True)
                elif R >= 10:
                    low = 0;
                    high = R
                    while low != high:
                        mid = (low + high) // 2
                        P, sP = greedyAppReduceAll(G, W, V, root, 1, mid, 1e8, 0, True, True)
                        if R - sP < mid:
                            high = mid
                        else:
                            low = mid + 1
                    P, sP = greedyAppReduceAll(G, W, V, root, 1, low, 1e8, 0, True, True)


                    if low > 1:
                        P_, sP_ = greedyAppReduceAll(G, W, V, root, 1, low - 1, 1e8, 0, True, True)
                    ...
                    P, sP = greedyAppReduceAll(G, W, V, root, 1, 1, 1e8, 0, True, True)
...
if __name__ == "__main__":
    ...
    # out = {1003950, 15004, 15131, 1003744, 15190, 1003746, 13730, 1003167, 13731, 13732, 13733, 14062, 1003206, 16503,
    #        13735, 16094, 16095, }
    #
    # out = {15190, 1003206, 16503, 15004, 14062, 13735, 1003167, 1003744, 1003746, 1003950, 13730, 13731, 13732, 13733,
    #        15131, 16094, 16095, 131099}
    # out = {1003950, 15004, 15131, 1003744, 15190, 1003746, 13730, 1003167, 13731, 13732, 13733, 14062, 1003206, 16503,
    #        13735, 16094, 16095}.union(out)



    #path = '../data/graph_geom_corrected_cycles.csv'
    path = '../simulate/graph_geom_corrected_cycles.csv'
    csv_grafo = pd.read_csv(path, sep=';')  # , sep=','
    ...
    G_ = G
    max_depth = 60
    #G = G_.subgraph([node for node in G_.nodes() if nx.shortest_path_length(G_, source=node, target=744) <= max_depth]);
    root = 744
    spanning = nx.algorithms.minimum_spanning_tree(G.to_undirected());
    Tree = root_graph(spanning, root)
    G = Tree

    print("\n Simulando: Entropy \n")
    T_entropy = simulate_entropy_minimization(G, root)
    with open('iters_entropy_tree.txt', "a") as myfile:
        myfile.write(' '.join([str(i) for i in T_entropy]) + '\n')
    print("\n Simulando: Path separation \n")
    T_paths = simulate_path_separation(G, root)
    with open('iters_paths_tree.txt', "a") as myfile:
        myfile.write(' '.join([str(i) for i in T_paths]) + '\n')
    print("\n Simulando: Greedy \n")
    T_greedy, map_prev = simulate_robust_randtree(G, root, {})
    with open('iters_greedy_tree.txt', "a") as myfile:
        myfile.write(' '.join([str(i) for i in T_greedy]) + '\n')
    print("\n Simulando: Treewidth \n")
    T_treewidth = simulate_treewidth_algorithm(G, root)
    with open('iters_treewidth_tree.txt', "a") as myfile:
        myfile.write(' '.join([str(i) for i in T_treewidth]) + '\n')
    print("\n Simulando: Optimal function \n")
    T_optimal = get_optimal_function(Tree, int(1e6))
    optimal = max(T_optimal.values())
    print(f'OPTIMAL = {optimal}')


    bins = [b + 0.5 for b in range(1, max(T_treewidth) + 1)]  # max(T_treewidth)
    fig = plt.figure()
    ax = fig.add_subplot(111)
    ax.hist(T_treewidth, bins=bins, alpha=0.7, label='Treewidth')
    ax.hist(T_paths, bins=bins, alpha=0.7, label='Paths')
    ax.hist(T_entropy, bins=bins, alpha=0.7, label='Entropy')
    ax.hist(T_greedy, bins=bins, alpha=0.7, label='Greedy')
    ax.axvline(optimal, color='k', linestyle='dashed', linewidth=1, label="Optimal value")
    ax.legend()
    # ax.xticks([b for b in range(1, max(T) + 1)])
    ax.set_xlim([0, max(T_treewidth) + 1])  # max(T_treewidth)
    plt.savefig("hist_algorithms_tree.pdf", format="pdf", bbox_inches="tight")
